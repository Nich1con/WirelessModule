/*
    ...
    Версии:
    v3.0
	- Библиотека переехала на шаблон! Старые примеры НЕСОВМЕСТИМЫ!
*/
...
class MicroDS18B20 {
  public:
    ...
    void setResolution(uint8_t resolution) {       		// Установка рабочего разрешения
    ...
    void readAddress(uint8_t *addressArray) {               // Чтение адреса датчика в массив
      ...
      for (uint8_t i = 0; i < 8; i++) {                 	// Прочитать 8 байт адреса
        ...
        _calculated_crc = crc_update(_calculated_crc, _temp_address[i]);  // Обновить значение CRC8
      }
    ...
    uint16_t getRaw(void) {                				// Прочитать "сырое" значение температуры
      ...
      for (uint8_t i = 0; i < 9; i++) {                 // Считать RAM
        ...
        _calculated_crc = crc_update(_calculated_crc, data[i]); // Обновить значение CRC8
      }
    ...
    DS_TEMP_TYPE calcRaw(uint16_t data) {             	// Преобразовать "сырое" значение в температуру
      return (DS_TEMP_TYPE) (data / 16);                // Рассчитать значение температуры
    }

    DS_TEMP_TYPE getTemp(void) {                		// Чтение температуры
      return (DS_TEMP_TYPE) calcRaw(getRaw());
    }
